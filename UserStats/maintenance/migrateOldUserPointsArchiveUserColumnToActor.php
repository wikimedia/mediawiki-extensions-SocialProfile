...
class MigrateOldUserPointsArchiveUserColumnToActor extends LoggedUpdateMaintenance {
	...
	protected function doDBUpdates() {
		...
		$dbw->query(
			"UPDATE {$dbw->tableName( 'user_points_archive' )} SET up_actor=(SELECT actor_id FROM {$dbw->tableName( 'actor' )} WHERE actor_user=up_user_id AND actor_name=up_user_name)",
			__METHOD__
		);
		return true;
	}
